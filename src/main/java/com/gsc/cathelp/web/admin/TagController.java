...
public class TagController {
    ...
    public String post(@Valid Tag tag,BindingResult result, RedirectAttributes attributes){
        ...
        if(tag1 != null){
            result.rejectValue("name","nameError","分类重复");
        }
        if(result.hasErrors()){
    ...
    public String editTagInput(@PathVariable Long id,Model model){
        model.addAttribute("tag",tagService.getTag(id));
        return "admin/tags-input";
    }

    @PostMapping("/tags/{id}")
    public String editTagPost(@Valid Tag tag,BindingResult result,@PathVariable Long id, RedirectAttributes attributes){
        Tag tag1 = tagService.getTagByName(tag.getName());
        if(tag1 != null){
            result.rejectValue("name","nameError","分类重复");
        }
    ...
    public String delete(@PathVariable Long id,RedirectAttributes attributes){
        ...
        attributes.addFlashAttribute("message","已删除");
        return "redirect:/admin/tags";
    }
